...
public class TraderPager {
    ...
    public static CompoundTag toNBT(RoleTrader role) {
        List<NpcMiscInventory> c = CURRENCY.get(role);
        ...
        for (int i = 0; i < c.size(); i++) {
            ...
            p.put("Tc", c.get(i).getToNBT());
            p.put("Ts", SOLD.get(role).get(i).getToNBT());
            pages.add(p);
        }
    ...
    public static void fromNBT(RoleTrader role, CompoundTag compound) {
        ...
        if (full && !pages.isEmpty()) {
            for (Tag tag : pages) {
                ...
                ci.setFromNBT(p.getCompound("Tc"));
                si.setFromNBT(p.getCompound("Ts"));
                c.add(ci);
                s.add(si);
            }

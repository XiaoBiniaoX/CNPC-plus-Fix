...
/**
 * Pure matching against InventoryCrafting grid (4x4 carpentry / generic).
 */
public final class RecipeMatcher {
    private RecipeMatcher() {}

    public static MatchResult findMatching(InventoryCrafting inv, Collection<RecipeCarpentry> candidates) {
        ...
        for (RecipeCarpentry recipe : candidates) {
            ...
            if (hit.hit()) {
                CnpcPlus.LOGGER.info("[RecipeMatcher] hit name={} offset={},{} grid={}x{}",
                        recipe.name, Integer.valueOf(hit.offsetX), Integer.valueOf(hit.offsetY),
                        Integer.valueOf(gridW), Integer.valueOf(gridH));
                return hit;
            }
        }
    ...
    private static MatchResult matchOne(RecipeCarpentry recipe, InventoryCrafting inv, int gridW, int gridH) {
        int rw = Math.max(1, recipeWidth(recipe));
        int rh = Math.max(1, recipeHeight(recipe));
    ...
    private static boolean checkMatch(RecipeCarpentry recipe, InventoryCrafting inv,
    ...
    private static int recipeWidth(RecipeCarpentry recipe) {
        return recipe.getRecipeWidth();
    }

    private static int recipeHeight(RecipeCarpentry recipe) {

 ...
 *
 * 存在的理由：{@code DataTransform} 没有这个字段（只有 {@code editingModus}
 * 与 {@code isActive}），而原版按钮 11/12 发的 {@code TransformLoad} 是
 * 一次性动作包，服务端不回传"当前在编辑哪一套"。详见
 * {@link bin.cnpcplus.mixin.advanced.MixinGuiNPCNightSetupTitle} 的类注释。
 *
 * <h3>为什么是纯客户端的单个静态字段</h3>
 * 这只是给编辑者看的界面提示，不参与任何数据判定，也不需要落盘或同步。
 * 同一时刻只可能有一个玩家在一个界面里编辑，所以不需要按 NPC 分表。
 * 刻意不放在 mixin 包内（阶段 22 的 {@code IllegalClassLoadError} 教训）。
 *
 * <h3>默认值</h3>
 * 默认 false（日）。原版界面在编辑模式开启时并不预先加载任何一套，
 * 玩家必须先点一次按钮才有意义；在他点之前显示"日模式"是保守选择。
 */

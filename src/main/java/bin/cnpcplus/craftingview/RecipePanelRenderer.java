...
/**
 * 1:1 port of 1.20.1 cnpcplus RecipePanelRenderer (layout + craftable bar + overlay).
 */
public final class RecipePanelRenderer {
    ...
    public static void render(GuiGraphics gr, Screen screen, RecipePanel panel, int mouseX, int mouseY, int guiLeft, int guiTop) {
        ...
        if (panel.getFilteredSize() > panel.getScrollOffset() + panel.getVisiblePerPage()) {
            gr.drawString(font, "\u25BC", px + pw / 2 - 3, cy, COLOR_TEXT_DIM, false);
        }

        // selected recipe overlay ON TOP of rows (z=200)
    ...
    private static int drawHeader(GuiGraphics gr, int cx, int px, int cy, int pw, RecipePanel panel,
        ...
        gr.drawString(font, Component.translatable(titleKey).getString(), cx, cy, COLOR_TEXT, false);
        cy += 10;

        panel.searchField.setX(cx);
        panel.searchField.setY(cy);
        panel.searchField.setWidth(pw - PADDING * 2);
        panel.searchField.setHint(Component.translatable("cnpcplus.craftingview.search"));
    ...
    private static ItemStack drawRecipeRow(GuiGraphics gr, int cx, int px, int ry, int pw, RecipeView recipe,
        ...
        if (result != null && !result.isEmpty()) {
            ...
            gr.renderItemDecorations(font, result, cx, ry);
        ...
        gr.drawString(font, font.plainSubstrByWidth(displayName, pw - PADDING * 2 - 18 - 14), cx + 18, ry + 4, COLOR_TEXT, false);
        ...
        gr.drawString(font, "+", btnX + 2, ry + 4, COLOR_TEXT, false);

        // green vertical craftable bar (respects fuzzy/name-only via panel.isCraftable)
    ...
    private static ItemStack drawIngredientOverlay(GuiGraphics gr, int px, int pw, int rowY, RecipeView recipe,
        ...
        int cx = ox + PADDING;
        int cy = oy + PADDING;
        gr.drawString(font, Component.translatable("cnpcplus.craftingview.preview").getString(), cx, cy, COLOR_TEXT_DIM, false);
        cy += 10;
        ...
        for (int row = 0; row < gridSize; row++) {
            for (int col = 0; col < gridSize; col++) {
                ...
                if (ing != null && !ing.isEmpty()) {
                    ...
                    gr.renderItemDecorations(font, ing, gx, gy);
    ...
    private static void drawCollapsedTab(GuiGraphics gr, int x, int y, int mouseX, int mouseY, Font font) {
        ...
        gr.drawString(font, ">", x + 3, y + 6, COLOR_TEXT, false);
    }
    ...
    private static void drawCollapseButton(GuiGraphics gr, int x, int y, int mouseX, int mouseY, Font font) {
        ...
        gr.drawString(font, "<", x + 2, y + 2, COLOR_TEXT, false);
    }
    ...
    /** Overlay sits BELOW the selected row (1.20.1 behavior). */
    ...
    public static boolean isPanelHit(RecipePanel panel, int guiLeft, int guiTop, int mx, int my) {
        ...
        int px = panel.getPanelX(guiLeft);
        int ph = calcPanelHeight(panel);
        // include overlay area so clicks on overlay count as panel
        int maxH = ph;
        int selIdx = panel.getSelectedVisibleIndex();
        if (selIdx >= 0) {
            RecipeView sel = panel.getSelectedRecipe();
